...
type DWorkspaceSummary = import('@process/decision/types').DecisionWorkspaceSummary;
...
export const decision = {
  ...
  workspace: {
    ...
    listSummary: bridge.buildProvider<DWorkspaceSummary[], { userId: string }>('decision.workspace.list-summary'),
    create: bridge.buildProvider<DWorkspace, { userId: string; name: string; description?: string }>(
      'decision.workspace.create'
    ),
  ...
  session: {
    ...
    create: bridge.buildProvider<
      { session: DSession; stageRun: DStageRun },
    ...
    getWithStages: bridge.buildProvider<{ session: DSession; stageRuns: DStageRun[] } | null, { id: string }>(
      'decision.session.get-with-stages'
    ),
    recent: bridge.buildProvider<DSession[], { userId: string; limit?: number }>('decision.session.recent'),
    advance: bridge.buildProvider<
      { session: DSession; stageRun: DStageRun },
      { sessionId: string; conversationId: string }
    >('decision.session.advance'),
    revert: bridge.buildProvider<
      { session: DSession; stageRun: DStageRun },
      { sessionId: string; targetStage: string; conversationId: string }
    >('decision.session.revert'),
    skip: bridge.buildProvider<
      { session: DSession; stageRun: DStageRun },
      { sessionId: string; conversationId: string }
    >('decision.session.skip'),
  ...
  stage: {
    ...
    getCompletionStatus: bridge.buildProvider<DCompletionStatus, { sessionId: string; stage: string }>(
      'decision.stage.completion-status'
    ),
    update: bridge.buildProvider<DStageRun, { id: string; updates: Partial<DStageRun> }>('decision.stage.update'),
    getByConversation: bridge.buildProvider<DStageRun | null, { conversationId: string }>(
      'decision.stage.get-by-conversation'
    ),
  },
  // ResearchItem
  research: {
    ...
    update: bridge.buildProvider<DResearchItem, { id: string; updates: Partial<DResearchItem> }>(
      'decision.research.update'
    ),
  ...
  evidence: {
    ...
    add: bridge.buildProvider<DEvidence, Omit<DEvidence, 'id' | 'createdAt'> & { sessionId: string }>(
  ...
  recommendation: {
    ...
    create: bridge.buildProvider<DRecommendation, Omit<DRecommendation, 'id' | 'createdAt'>>(
      'decision.recommendation.create'
    ),
    update: bridge.buildProvider<DRecommendation, { id: string; updates: Partial<DRecommendation> }>(
      'decision.recommendation.update'
    ),
  },
  // Insight
  insight: {
    list: bridge.buildProvider<DInsight[], { sessionId: string }>('decision.insight.list'),
    listByStage: bridge.buildProvider<DInsight[], { sessionId: string; stage: string }>(
      'decision.insight.list-by-stage'
    ),
  ...
  analytics: {
    ...
    contextDetail: bridge.buildProvider<Record<string, unknown>, { sessionId: string; stage: string }>(
      'decision.analytics.context-detail'
    ),
